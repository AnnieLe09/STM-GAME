...
public class UserHandler extends BaseClientRequestHandler {
        ...
        try {
            switch (dataCmd.getId()) {
                case CmdDefine.GET_USER_INFO:
                    getUserInfo(user);
                    break;
                case CmdDefine.RESET_USER_INFO:
                    resetUserInfo(user);
                    break;
    ...
    private void resetUserInfo(User user) {
        try {
            PlayerInfo userInfo;
            userInfo = new PlayerInfo(user.getId(), "Fresher_" + user.getId());
            userInfo.saveModel(user.getId());
            user.setProperty(ServerConstant.PLAYER_INFO,userInfo);

            user.disconnect(new IDisconnectionReason() {
                @Override
                public int getValue() {
                    return 0;
                }
